...
class SelectedGenreBooks: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var category = ""
        if(intent.extras!!.getString("bookCategory") == "No category"){
            category = "null"
        }
        else{
            category = intent.extras!!.getString("bookCategory").toString()
        }
        println(category)

        val recyclerView = findViewById<RecyclerView>(R.id.recyclerView)
        recyclerView.layoutManager = LinearLayoutManager(this)
        retrieveAuthToken(recyclerView,shelfType,category)

    }


  private fun retrieveAuthToken(layout: RecyclerView, shelfType:String?, category: String?) {
